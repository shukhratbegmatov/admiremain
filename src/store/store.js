...
export default new Vuex.Store({
  ...
  actions: {
      ...
      axios
        .get(
          "market/" +
            router.currentRoute.params.id +
            "/acts/" +
            router.currentRoute.params.pro_id +
            "?_l=en",
          {
            headers: {
              "Content-Type": "application/json",
    ...
    getMagazin({ commit, state }, credentials) {
      state.pending = true;
      axios
        .get("market?per-page=" + credentials, {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + localStorage.getItem("token"),
          },
    ...
    getProduct({ commit }) {
      axios
        .get("market/" + router.currentRoute.params.id, {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + localStorage.getItem("token"),
          },
    ...
    createProduct(_, credentials) {
      return axios.post(
        "market/" + router.currentRoute.params.id + "/new-action",
        credentials,
        {
          headers: {
            "Content-Type": "application/json",
    ...
    getLink({ state }) {
      axios
        .get("/market/" + router.currentRoute.params.id + "/get-link", {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + localStorage.getItem("token"),
          },
