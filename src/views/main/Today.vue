  ...
  <div>
    <main class="p-b-110">
        ...
        <div class="container">
          <div class="today-balance">
            <div class="today-last">
              ...
              <div>
                <span> {{ $store.state.data }} </span><span>so'm</span>
              </div>
            </div>
            <div class="today-first">
              <img src="@/assets/images/svg/down-blue.svg" alt=":(" />
              <div><span></span> <span>so'm</span></div>
            </div>
          </div>
        </div>
        <div class="bg-gray">
      ...
      <div class="scroll-body">
        <market-item
          isModal
          v-for="marketItem in $store.state.statistics"
          :key="marketItem.id"
          ...
          <div class="modalContent">
            <div class="modal-market">
              <div class="market-logo">
                <img src="@/assets/images/png/Makro.png" alt=":(" />
              </div>
              <div>
                <div class="modal-market-name">
                  {{ $store.state.is_showStatistic.name }}
                </div>
                <div class="today-time">
                  {{
                    new Date(
                      $store.state.is_showStatistic.time * 1000
                    ).toLocaleTimeString()
                  }}
                </div>
              </div>
            ...
            <div class="modal-client-block">
              <div class="modal-client-info">
                ...
                <span class="b-text-b">Diller nomi:</span>
              </div>
              <div class="modal-client-name">
                {{ $store.state.is_showStatistic.username }}
              </div>
            </div>
            <div class="modal-client-block">
              <div class="modal-client-info">
                ...
                <span class="b-text-b">Tel raqam::</span>
              </div>
              <div class="modal-client-name">
                {{ $store.state.is_showStatistic.phone }}
              </div>
            </div>
            <div class="modal-footer">
              <span
                @click="$store.state.isActive = false"
                class="modalCloseButton"
                >Ortga</span
              >
              <span class="m-f-line"> </span>
              <a :href="'tel:' + $store.state.is_showStatistic.phone"
                >Qo'ng'iroq</a
              >
            </div>
          </div>
...
import marketItem from "@/views/magazin/ItemMarket.vue";
export default {
  data() {
    return {
      markets: [{}],
