<template>
  <div @scroll="scroll">
      ...
      <div class="search" :class="{'active':searchs}">
        <div class="search_ch">
          <div class="input_group">
            <input type="text" id="search"  @input="searchsF()" v-model="text" placeholder="Maxsulot nomi bilan izlang">

            <svg @click="searchs=false;text=''" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <main class="main">
      <div class="container" >
        <div>
          <a  @click="nexPage(item.id)" @scroll="scroll(item)" :id="'id'+item.id"  @mousedown="getDelete($event)" class="market-item" v-for="(item,index) in markets.data" :key="index">
            <div>
              <div class="market-client">
                ...
                <div>
                  ...
                  <div class="market-price"><span class="market-back">
                <img src="../../assets/images/svg/back.svg" alt="icon"></span>
                    <div class="market-sum"><span>{{ item.market_balance }}</span><span> so'm</span>
                    </div>
                  </div>
                </div>
              </div>
...
export default {
  ...
  mounted() {
    axios.get("market?per-page="+this.item,{
      headers: {
  ...
  methods: {
    searchsF(){
      axios.get("market?name="+this.text,{
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + localStorage.getItem('token')
        }
